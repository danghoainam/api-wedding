...
final class Hash
{
    ...
    public static function encrypt(string $str): string
    {
        ...
        $encrypted = null;
        if (isset($key[1])) {
            $decodedKey = base64_decode($key[1]);
            $encrypted = openssl_encrypt($str, static::CIPHERING, $decodedKey, OPENSSL_RAW_DATA, $iv);
        } else {
            // Xử lý khi mảng $key không có phần tử thứ nhất
            // Ví dụ: đưa ra thông báo lỗi, hoặc xử lý khác tùy thuộc vào logic của ứng dụng.
        }
